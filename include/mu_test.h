 ...
 * to be unable to not partly misunderstand this!!!
 */
...
#define KBCYN 96
#define KULN  4
enum MT_Cats{MT__V_MARK,MT__X_MARK,MT__GRIN_FACE,MT__ANGUISH,MT__GRIMACE,MT__LOUD_CRY,MT__SCREAM,MT__CAT_GRIN,MT__CAT_GRIN_SMILE,MT__CAT_JOY,MT__CAT_POUTING,MT__CAT_SAD,
MT__CAT_WAERY,MT__STOP_WATCH,MT__TIMER_CLOCK,MT__HOUR_GLASS,MT__CLOCK_FACE_2_OCLOCK,MT__THUMBS_UP,MT__THUMBS_DOWN,MT__PASS_EMJ=MT__GRIN_FACE,MT_FAIL_ICN=MT__X_MARK,MT_PASS_ICN=MT__V_MARK};MT_SYMBOL Catz MT_WITH
...
#define IGNORE_TEST_IMPL(unit)   {unit, MT_CLR(KYEL) #unit, 1},
...
#define mt__ACTUALL  MT_CLR(KCYN) "actual  : " MT_CLR(KRED)
static char* mt__EXPECTED_i(long a,long e,MT_SYMBOL xx,MT_SYMBOL zz){char* p = (char*) malloc(1024);if(p) sprintf(p,mt__ACTUALL "%ld\n\t\t\t" mt__EXPECTED "%ld",a,e);(void)xx;(void)zz;MT__GIVE p;}
static char* mt__EXPECTED_s(MT_SYMBOL a, MT_SYMBOL e,MT_SYMBOL xx,MT_SYMBOL zz){char*p;if(!a)return 0;p=(char*) malloc(1024);if(p)sprintf(p,mt__ACTUALL "'%s'\n\t\t\t"  mt__EXPECTED "'%s'",a,e);(void)xx;(void)zz;MT__GIVE p;}
static char* mt__EXPECTED_p(const volatile void*volatile const a,const volatile void*volatile const e,MT_SYMBOL xx,MT_SYMBOL zz){char* p = (char*) malloc(1024);if(p) sprintf(p,mt__ACTUALL "%p\n\t\t\t"  mt__EXPECTED "%p",a,e);(void)xx;(void)zz;MT__GIVE p;}
static void mt__trace_logw(int nl,MT_SYMBOL fmt,va_list arg){if(!mt__trace_on)return;if(!mt__current_test_was_traced){mt__current_test_was_traced=1;M__FP MT_CLR(KBCYN) "\tTracing    -    " MT_CLR(KBBLU) "%s\n", mt__current_test_name);}M__FP MT_CLR(KNRM));M__VFP fmt,arg);M__FP MT_CLR(KNRM));M__FP nl?"\n":"");fflush(stdout);}
...
#ifdef __cplusplus
#include <sstream>
template<typename A,typename B>inline const char*mt_cxx_Expected(A const&a,B const&e){std::stringstream ss;ss<<mt__ACTUALL<<a<<"\n\t\t\t"<<mt__EXPECTED<<e;char*p=(char*)malloc(1+ss.tellp());strcpy(p,ss.str().c_str());MT__GIVE p;}
#define ASSERT_NOT_EQUAL_CXX_IMPL(a,e) mt__STATEMENTS({mt__ASSERT(a!=e,__LINE__, "ASSERT_EQUAL(" #a ", " #e ")", mt_cxx_Expected(a,e)); }) 
#define ASSERT_EQUAL_CXX_IMPL(a,e) mt__STATEMENTS({mt__ASSERT(a==e,__LINE__, "ASSERT_EQUAL(" #a ", " #e ")", mt_cxx_Expected(a,e)); }) 

class MT_Tracer {
public:
    template<typename T>
    MT_Tracer& trace(T const& o){
        if(!mt__trace_on){return *this;}if(!mt__current_test_was_traced){mt__current_test_was_traced=1;M__FP MT_CLR(KBCYN) "\tTracing    -    " MT_CLR(KBBLU) "%s\n", mt__current_test_name);}
        std::cout << MT_CLR(KNRM) << o << MT_CLR(KNRM);
        return *this;
    }
};
template<typename T>
MT_Tracer& operator<<(MT_Tracer& tm, const T& obj){
    return tm.trace(obj);
}
MT_Tracer TRACER;
...
#define ASSERT_THAT_IMPL(e)              mt__STATEMENTS({int _e = !!(e); mt__ASSERT((_e),__LINE__,"ASSERT_THAT(" #e ")", mt__EXPECTED_s(0,0,0,0)); })

/* Readers shall not be disallowed from not failing to be unable to not partly misunderstand this */

#define MU_TEST_PUBLIC
